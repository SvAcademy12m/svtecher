...
const JobFinderDashboard = () => {
  ...
  const navSections = [
    ...
    },
    {
      title: 'PROFILE',
      items: [
        { key: 'resume', label: 'Resume / CV', icon: HiBriefcase },
  ...
  const SidebarContent = ({ onNavClick }) => (
    <div className="flex flex-col h-full bg-[#0e1225]" style={{ background: 'linear-gradient(180deg, #0e1225 0%, #17101c 50%, #0e1225 100%)' }}>
      <div className="p-4 pb-3 flex items-center gap-3 border-b border-white/5">
        ...
        <div>
          <h1 className="text-sm font-black text-white tracking-tight">Career Portal</h1>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-3 overflow-y-auto w-[240px]">
        {navSections.map((section, si) => (
          <div key={si} className={si > 0 ? 'mt-5' : ''}>
            {section.title && (
              <p className="px-3 mb-1.5 text-[10px] font-black text-amber-300/60 uppercase tracking-[0.2em]">{section.title}</p>
            )}
              ...
              {section.items.map(item => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all duration-200 ${
                    activePanel === item.key ? 'bg-amber-500/20 text-white border border-amber-400/20 shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <item.icon className={`w-[18px] h-[18px] flex-shrink-0 ${activePanel === item.key ? 'text-amber-400' : ''}`} />
      ...
      <div className="px-4 py-3 border-t border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-amber-500 to-orange-400 flex items-center justify-center text-white text-[9px] font-bold">
            {getInitials(userData?.name)}
          </div>
          <div className="overflow-hidden">
            <p className="text-[11px] font-bold text-white truncate">{userData?.name}</p>
            <p className="text-[9px] text-slate-500 truncate">Job Finder</p>
          </div>
        </div>
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-white border-b border-slate-100 px-4 lg:px-6 flex items-center justify-between flex-shrink-0">
          ...
          <button onClick={() => navigate('/jobs')} className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 text-xs font-bold text-amber-600 hover:bg-amber-100 transition-colors">
             <HiOfficeBuilding className="w-4 h-4" /> Find Jobs
          </button>
        </header>
          ...
          {activePanel === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
                <div className="bg-gradient-to-br from-amber-500 to-orange-500 rounded-2xl p-4 md:p-5 text-white shadow-lg stat-shine">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-white/20 flex items-center justify-center mb-3">
                    <HiDocumentText className="w-4 h-4 md:w-5 md:h-5 text-white" />
                  </div>
                  <p className="text-2xl md:text-3xl font-black">{applications.length}</p>
                  <p className="text-[9px] md:text-[10px] font-bold text-amber-100 uppercase tracking-wider mt-1">Applications</p>
                </div>
                <div className="bg-white rounded-2xl border border-slate-100 p-4 md:p-5 shadow-sm relative overflow-hidden">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-emerald-50 flex items-center justify-center mb-3">
                    <HiCheckCircle className="w-4 h-4 md:w-5 md:h-5 text-emerald-500" />
                  </div>
                  <p className="text-2xl md:text-3xl font-black text-slate-900">{applications.filter(a => a.status === 'accepted').length}</p>
                  <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Interviews</p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-4 md:p-5 shadow-sm">
                  ...
                  <p className="text-2xl md:text-3xl font-black text-slate-900">120</p>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-4 md:p-5 shadow-sm relative overflow-hidden">
                  ...
                  <p className="text-2xl md:text-3xl font-black text-slate-900">Level 2</p>
              ...
              {/* Applications List */}
              <div className="mt-8">
                <h4 className="font-bold text-slate-900 mb-4">Recent Applications</h4>
                <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="bg-slate-50/80 border-b border-slate-100">
                          <th className="text-left p-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Position</th>
                          <th className="text-left p-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Company</th>
                          <th className="text-left p-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Date Applied</th>
                          <th className="text-center p-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {applications.map(app => (
                          <tr key={app.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                            <td className="p-4 font-semibold text-sm text-slate-900">{app.jobTitle || 'Unknown Job'}</td>
                            <td className="p-4 text-xs font-medium text-slate-600">SVTech Network</td>
                            <td className="p-4 text-xs text-slate-400">{app.createdAt ? new Date(app.createdAt?.seconds * 1000).toLocaleDateString() : '—'}</td>
                  ...
                  {applications.length === 0 && (
                    <div className="text-center py-16 bg-white">
                      <HiBriefcase className="w-12 h-12 text-slate-200 mx-auto mb-3" />
                      <p className="text-sm font-medium text-slate-500 mb-4">You haven't applied to any jobs yet.</p>
                      <button onClick={() => navigate('/jobs')} className="px-5 py-2.5 bg-amber-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-amber-500/30 hover:bg-amber-600 transition-all">
                        Find Jobs
                      </button>
                    </div>
                  )}
